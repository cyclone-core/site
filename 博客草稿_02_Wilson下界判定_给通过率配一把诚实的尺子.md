# 博客草稿 #2：Wilson 下界判定——给通过率配一把诚实的尺子

> 目标读者：测试经理 / 测试主管（同上篇，不需要统计学背景）
> 看完做什么：了解 Cyclone Core 判定引擎（verdict engine）
> 诚实标签：产品段落已过「已支持 / 规划中」检查；数值示例为统计学计算结果
> 状态：可发布（通俗版，文末附名词解释）
> 系列位置：第 2 篇。上篇《跑 30 把过 27 把，为什么不算验证通过》讲"为什么要看保底分"，本篇讲"保底分怎么算、用哪把尺子量"
